...
final class KitSavedPlacesScreen extends KitHudScreen {
	...
	protected void init() {
		...
		placeName = addRenderableWidget(KitUi.field(this.font, left, bodyTop(42), 120, "地点名称", draftName, 32));
		targetX = addRenderableWidget(KitUi.field(this.font, left + 128, bodyTop(42), 72, "目标 X",
			draftX != null ? draftX : KitUi.formatNumber(config.targetX), 24));
		targetZ = addRenderableWidget(KitUi.field(this.font, left + 208, bodyTop(42), 72, "目标 Z",
			draftZ != null ? draftZ : KitUi.formatNumber(config.targetZ), 24));
		cruiseY = addRenderableWidget(KitUi.field(this.font, left + 288, bodyTop(42), 72, "巡航 Y",
		...
		addRenderableWidget(Button.builder(Component.literal("新增 / 更新"), button -> savePlace())
			.bounds(left, bodyTop(68), 88, 20)
			.tooltip(Tooltip.create(Component.literal("同名地点会覆盖坐标；回车也可保存")))
			.build());
		addRenderableWidget(Button.builder(Component.literal("填入当前位置"), button -> useCurrentPosition())
			.bounds(left + 92, bodyTop(68), 88, 20).build());
		addRenderableWidget(Button.builder(Component.literal("设为当前目标"), button -> applyAsTarget())
			.bounds(left + 184, bodyTop(68), 88, 20)
			.tooltip(Tooltip.create(Component.literal("只保存到主界面目标，不立刻起飞")))
			.build());
		addRenderableWidget(Button.builder(Component.literal("维度：" + KitConfig.dimensionLabel(draftDimension)), button -> {
			...
			rebuildWidgets();
		}).bounds(left + 276, bodyTop(68), 84, 20)
			.tooltip(Tooltip.create(Component.literal("这个地点属于哪个维度。填入当前位置会改成你现在所在的维度。")))
			.build());

		int filterY = bodyTop(92);
		addFilterButton(left, filterY, 88, KitConfig.DIM_OVERWORLD);
		addFilterButton(left + 92, filterY, 88, KitConfig.DIM_NETHER);
		addFilterButton(left + 184, filterY, 88, KitConfig.DIM_END);
		addFilterButton(left + 276, filterY, 84, FILTER_ALL);

		listSearch = addRenderableWidget(KitUi.field(this.font, left, filterY + 24, 360, "搜索名称",
			listSearchText, 32));
		...
		int listTop = bodyTop(142);
		int listHeight = Math.max(48, contentBottom() - 28 - listTop);
		list = addRenderableWidget(new PlaceList(this.minecraft, this.width, listHeight, listTop));
		list.populate();
		...
		if (guide != null && guide.isActive()) {
			addRenderableWidget(Button.builder(Component.literal("停指引"), button -> {
				...
				rebuildWidgets();
			}).bounds(this.width / 2 - 158, contentBottom(), 80, 20).build());
			addRenderableWidget(Button.builder(Component.literal("返回"), button -> onClose())
				.bounds(this.width / 2 - 70, contentBottom(), 150, 20).build());
		} else {
			addRenderableWidget(Button.builder(Component.literal("返回"), button -> onClose())
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
		...
		if (!notice.isEmpty()) KitUi.centered(graphics, this.font, notice, center, headerY(20), noticeColor);
		KitUi.text(graphics, this.font, "名称", left, bodyTop(32), 0xFFFFFF);
		KitUi.text(graphics, this.font, "X", left + 128, bodyTop(32), 0xFFFFFF);
		KitUi.text(graphics, this.font, "Z", left + 208, bodyTop(32), 0xFFFFFF);
		KitUi.text(graphics, this.font, "巡航 Y", left + 288, bodyTop(32), 0xFFFFFF);
		if (config.savedPlaces.isEmpty()) {
			KitUi.centered(graphics, this.font, "还没有保存地点；填好名称和坐标后点「新增 / 更新」", center, bodyTop(148), 0xA0A0A0);
		} else if (countPlaces(listFilter) == 0 && !FILTER_ALL.equals(listFilter)) {
			KitUi.centered(graphics, this.font,
				KitConfig.dimensionLabel(listFilter) + "还没有地点，可点「全部」看其它维度", center, bodyTop(148), 0xA0A0A0);
		}
	}
	...
	private void loadPlace(KitConfig.SavedPlace place) {
		placeName.setValue(place.name);
		targetX.setValue(KitUi.formatNumber(place.x));
		targetZ.setValue(KitUi.formatNumber(place.z));
		cruiseY.setValue(KitUi.formatNumber(place.cruiseY));
		...
		private final class PlaceRow extends Row {
			...
			private PlaceRow(KitConfig.SavedPlace place) {
				...
				this.guide = Button.builder(Component.literal("指引"), button -> startGuide(place))
					...
					.build();
				this.edit = Button.builder(Component.literal("编辑"), button -> loadPlace(place)).bounds(0, 0, 36, 20).build();
			...
			@Override
			public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float delta) {
				String name = place.name.length() > 16 ? place.name.substring(0, 15) + "…" : place.name;
				String description = String.format(Locale.ROOT, "%s  X %.0f  Z %.0f  Y %.0f",
					name, place.x, place.z, place.cruiseY);
				graphics.text(font, description, getContentX(), getContentYMiddle() - 4, KitUi.argb(hovered ? 0xFFFF55 : 0xFFFFFF));
