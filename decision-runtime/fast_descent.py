...
def descend_if_clear(client, target_y, braking_margin=16):
    ...
    if initial.get('freefall_protocol',0)<1:
        return {'used':False,'reason':'Native frame brake is unavailable'}
    if initial['health']<18 or not initial.get('guard_armed') or initial['pos'][1]-brake_y<=24:
        return {'used':False,'reason':'No long, guarded descent required'}
    deadline=time.monotonic()+8
    previous=None;stable=0
    while True:
        # status.json updates about once a second; repeated reads can falsely
        # claim zero movement while Flight still drifts. Native snapshots are
        # fresh enough to prove the 3x3 clearance column is still under us.
        start=client.request('snapshot')
    ...
    latest=client.request('snapshot')
